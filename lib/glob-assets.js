...
const globAssets = async ({cwd}, assets) => {
  const results = await Promise.all(
    assets.map(async (asset) => {
      ...
      const directoryGlobResult = await dirGlob(glob, {cwd});
      glob = uniq([...directoryGlobResult, ...glob]);
  ...
  return uniqWith(
    results.flat().sort((asset) => (isPlainObject(asset) ? -1 : 1)),
    // Compare `path` property if Object definition, value itself if String
    (a, b) =>
      path.resolve(cwd, isPlainObject(a) ? a.path : a) ===
      path.resolve(cwd, isPlainObject(b) ? b.path : b),
  );
};

export default globAssets;
