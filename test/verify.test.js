...
import {authenticate} from './helpers/_mock-gitea.js';
...
test.serial('Verify package, token and repository access', async (t) => {
  ...
  await t.notThrowsAsync(
    verify(
      ...
      {
        env,
        options: {repositoryUrl: `git+https://gitea.io/${owner}/${repo}.git`},
...
test.serial(
  'Verify package, token and repository with environment variables',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {
    ...
    await t.notThrowsAsync(
      verify(
        ...
        {
          env,
          options: {repositoryUrl: `git@gitea.io:${owner}/${repo}.git`},
          logger: t.context.logger,
        },
      ),
    );
    t.true(gitea.isDone());
    t.assert(
      t.context.log.args[0],
      'Verify Gitea authentication (https://gitea.io/prefix)',
    );
  },
);

test.serial(
  'Verify package, token and repository access with alternative environment varialbes',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {
...
test.serial(
  'Verify "assets" is an Array of Object with a path property',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {GITEA_URL: 'https://gitea.io', GITEA_TOKEN: 'gitea_token'};
...
test.serial(
  'Verify "assets" is an Array of Object with a glob Arrays in path property',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {GITEA_URL: 'https://gitea.io', GITEA_TOKEN: 'gitea_token'};
...
test.serial('Throw SemanticReleaseError for invalid token', async (t) => {
  ...
  const repo = 'test_repo';
  const env = {GITEA_URL: 'https://gitea.io', GITEA_TOKEN: 'gitea_token'};
  const gitea = authenticate(env).get(`/repos/${owner}/${repo}`).reply(401);
  ...
  const aggregateError = await t.throwsAsync(
    verify(
      ...
      {
        env,
        options: {repositoryUrl: `https://gitea.io/${owner}/${repo}.git`},
        logger: t.context.logger,
      },
    ),
  );
  const [error, ...errors] = aggregateError.errors;
...
test('Throw SemanticReleaseError for invalid repositoryUrl', async (t) => {
  ...
  const aggregateError = await t.throwsAsync(
    verify(
      {},
      {env, options: {repositoryUrl: 'invalid_url'}, logger: t.context.logger},
    ),
  );
  const [error, ...errors] = aggregateError.errors;
...
test.serial(
  "Throw SemanticReleaseError if token doesn't have the push permission on the repository",
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {GITEA_URL: 'https://gitea.io', GITEA_TOKEN: 'gitea_token'};
...
test.serial(
  "Throw SemanticReleaseError if the repository doesn't exist",
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {GITEA_URL: 'https://gitea.io', GITEA_TOKEN: 'gitea_token'};
    const gitea = authenticate(env).get(`/repos/${owner}/${repo}`).reply(404);
...
test.serial('Throw error if Gitea return any other errors', async (t) => {
  ...
  const repo = 'test_repo';
  const env = {GITEA_URL: 'https://gitea.io', GITEA_TOKEN: 'gitea_token'};
  const gitea = authenticate(env).get(`/repos/${owner}/${repo}`).reply(500);
...
test.serial(
  'Throw SemanticReleaseError if "assets" option is not a String or an Array of Objects',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {GITEA_URL: 'https://gitea.io', GITEA_TOKEN: 'gitea_token'};
...
test.serial(
  'Throw SemanticReleaseError if "assets" option is an Array with invalid elements',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {GITEA_URL: 'https://gitea.io', GITEA_TOKEN: 'gitea_token'};
...
test.serial(
  'Throw SemanticReleaseError if "assets" option is an Object missing the "path" property',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {GITEA_URL: 'https://gitea.io', GITEA_TOKEN: 'gitea_token'};
