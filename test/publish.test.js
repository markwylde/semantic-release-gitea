...
test.serial('Publish a release with one asset', async (t) => {
  ...
  const giteaUpload = upload(env)
    .post(
      `/repos/${owner}/${repo}/releases/${releaseId}/assets?name=${encodeURIComponent('A dotfile with no ext')}`,
    )
...
test.serial(
  'Publish a release with a asset containing the next release version',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {GITEA_URL: 'https://gitea.io', GITEA_TOKEN: 'gitea_token'};
      ...
      assets: [
        {
          ...
          path: 'my-software-v${nextRelease.version}.tar.gz',
          // eslint-disable-next-line no-template-curly-in-string
          label: 'Version ${nextRelease.version} of my software',
        },
      ],
    };
    const nextRelease = {
    ...
    const giteaUpload = upload(env)
      .post(
        `/repos/${owner}/${repo}/releases/${releaseId}/assets?name=${encodeURIComponent('Version 1.0.0 of my software')}`,
      )
      .reply(200, {browser_download_url: assetUrl});
